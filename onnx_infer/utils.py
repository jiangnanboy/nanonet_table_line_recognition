...
def line_to_line(points1, points2, alpha=10):
    ...
    if (flag1 > 0 and flag2 > 0) or (flag1 < 0 and flag2 < 0):
        x = (B1 * C2 - B2 * C1) / ((A1 * B2 - A2 * B1) + 0.01)
        y = (A2 * C1 - A1 * C2) / ((A1 * B2 - A2 * B1) + 0.01)
        p = (x, y)
        r0 = sqrt(p, (x1, y1))
        r1 = sqrt(p, (x2, y2))
        if min(r0, r1) < alpha:
...
def solve(box):
    ...
    # x = cx-w/2
    # y = cy-h/2
    sinA = (h * (x1 - cx) - w * (y1 - cy)) * 1.0 / ((h * h + w * w) * 2 + 0.0001)
...
def count_info(func):
    import os
    def float_info(regions, flag=True, W=0, H=0, filtersmall=False, adjustBox=False):
        pid = os.getpid()
        p = psutil.Process(pid)
        info_start = p.memory_full_info().uss/1024
        func(regions, flag, W, H, filtersmall, adjustBox)
        info_end=p.memory_full_info().uss/1024
        print("minAreaRectbox程序占用了内存"+str(info_end-info_start)+"KB")
    return float_info
...
def minAreaRectbox(regions, flag=True, W=0, H=0, filtersmall=False, adjustBox=False):
    ...
    boxes = []
    print('regions_n: {}'.format(regions_n))

    # pool = Pool(processes=4)
    # for i in range(regions_n):
    #     box = pool.apply_async(thread_minAreaRectbox, (regions[i], flag, W, H, filtersmall, adjustBox))
    #     boxes.append(box.get())
    # pool.close()
    # pool.join()
    import time

    with concurrent.futures.ThreadPoolExecutor(max_workers=regions_n) as executor:
        to_do = []
        for i in range(len(regions)):
            future = executor.submit(thread_minAreaRectbox, regions[i], flag, W, H, filtersmall, adjustBox)
            to_do.append(future)
        for future in concurrent.futures.as_completed(to_do):
            if future.result() is not None:
                boxes.extend(future.result())
    print('boxes_n: {}'.format(len(boxes)))
    return boxes

def thread_minAreaRectbox(region, flag=True, W=0, H=0, filtersmall=False, adjustBox=False):
    boxes = []
